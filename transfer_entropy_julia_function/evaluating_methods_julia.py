...
def one_set_run(iter_values, defaults, to_target, metric_func, beta_decay = True, comparison_type = 'direct coordination',n_iter = 10):
    ...
    if beta_decay:
        while l>=0:
            ...
            res = run_compare_one_run(to_use, beta_decay, metric_func, comparison_type, res, n_iter)
...
def run_compare_one_run(to_use, beta_decay, metric_func, comparison_type, res, n_iter):
    ...
    for _ in range(n_iter):
        ...
        # filter times for only after t burnin
        ...
        to_save = [[k]+list(np.sort(v)) for k,v in times.items()]

        import csv
        with open("files/times.csv", "w") as f:
            writer = csv.writer(f)
            writer.writerows(to_save)

        # flag
        np.savetxt('files/python_flag.txt',[])

        # wait for julia
        print('waiting on julia...')
        wait_on_file('files','julia_flag.txt',20)
        # delete flag
        os.remove('files/julia_flag.txt')
        # read it back in
        B = np.genfromtxt("files/julia_estimates.csv", dtype=float, delimiter=',') 

        
        # comparison matrix
        if comparison_type == 'direct coordination':
            m_comp = to_use['A']
            res.append([to_use, t_lengths, matrix_similarity(m_comp,B, measure = 'lr', norm = True)])
            ...
            res.append([to_use, t_lengths, matrix_similarity(m1_comp,B, measure = 'lr', norm = True), matrix_similarity(m2_comp,B, measure = 'lr', norm = True)])
        else:
            print('Comparison type is not valid, please use either \'direct coordination\' or \'generating similarity\'.')

    return res

def assess_metric(metric_func, iter_values, defaults, beta_decay = True, comparison_type = 'direct coordination', n_iter = 10):
    ...
    res = []
    for to_target in tqdm(iter_values.keys()):
        res.append([one_set_run(iter_values, defaults, to_target, metric_func, beta_decay, comparison_type = comparison_type, n_iter = n_iter),to_target])
